    ...
    public class Balance
    {
        ...
        public Balance(XmlNode ledgerNode, XmlNode avaliableNode)
        {
            ...
            if (!String.IsNullOrEmpty(tempLedgerBalance))
            {
                ...
                var temp = tempLedgerBalance.Replace(".", "").Replace(",", "").Trim();
                LedgerBalance = Convert.ToDecimal(temp.Insert(temp.Length - 2, "."), CultureInfo.InvariantCulture);
            }
            else
            {
                LedgerBalance = 0M;
            ...
            else
            {
                ...
                if (!String.IsNullOrEmpty(tempAvaliableBalance))
                {
                    ...
                    var temp = tempAvaliableBalance.Replace(".", "").Replace(",", "");
                    AvaliableBalance = Convert.ToDecimal(temp.Insert(temp.Length - 2, "."), CultureInfo.InvariantCulture);
                }
                else
                {
